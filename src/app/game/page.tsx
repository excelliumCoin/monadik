...
export default function GamePage() {
  ...
  // horizontal lanes (0..2) and NEW: forward offset (0..MAX_FORWARD)
  const [playerLane, setPlayerLane] = useState(1);
  const [playerForward, setPlayerForward] = useState(0); // 0 = bottom, increases when moving forward

  // crash keeps score until saved
  const [pending, setPending] = useState<{ score: number; laps: number } | null>(null);

  // world clear hook (owned by CanvasStage)
  const clearWorldRef = useRef<() => void>(() => {});

  const MAX_FORWARD = 200;          // how far up the player can move
  const STEP_FORWARD = 28;          // per key press forward/back amount

  const startNewRun = () => {
    if (!walletLooksValid) return setMessage('Invalid wallet address.');
    clearWorldRef.current();      // wipe enemies / timers / offsets
    setScore(0);
    setLaps(0);
    setPlayerLane(1);
    setPlayerForward(0);          // reset forward position
    setMessage(null);
    setRunning(true);
  };
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (key === 'arrowleft' || key === 'a') {
        setPlayerLane((v) => Math.max(0, v - 1));
      }
      if (key === 'arrowright' || key === 'd') {
        setPlayerLane((v) => Math.min(2, v + 1));
      }
      if (key === 'arrowup' || key === 'w') {
        setPlayerForward((p) => Math.min(MAX_FORWARD, p + STEP_FORWARD));
      }
      if (key === 'arrowdown' || key === 's') {
        setPlayerForward((p) => Math.max(0, p - STEP_FORWARD));
      }
  ...
  // Save to chain (no user signature; server signer)
  const submit = async () => {
  ...
  return (
    <main style={{ maxWidth: 1400, margin: '0 auto', padding: 24 }}>
      ...
      {/* 3 columns: left=controls | center=game | right=stats */}
      <div className="row">
        {/* LEFT: Controls */}
        <aside className="panel side">
          ...
          <div className="hint">
            Move with ← → (A/D) and ↑ ↓ (W/S). Space starts a new run.
          </div>
...
/** Canvas scene with sprites, forward/back movement & tighter hitboxes */
function CanvasStage({
  ...
  running: boolean;
  playerLane: number;
  playerForward: number;           // 0..MAX_FORWARD from parent
  playerCarSrc?: string;
  enemyCarSrcs?: string[];
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      // collision with shrunken hitboxes
  ...
  const getPlayerRects = () => {
    ...
    const h = 84;
    const baseY = getH() - 78 - h / 2;        // original position near bottom
    const x = (laneW + laneGap) * playerLane + padding + laneW / 2 - w / 2;
    const y = baseY - playerForward;          // move up when forward increases
...
// fallback drawings (if sprites fail)
