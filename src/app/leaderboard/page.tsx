...
type ApiResp =
  | { ok: true; rows: Row[]; fromBlock?: number; toBlock?: number }
...
export default function LeaderboardPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!data.ok) throw new Error((data as any).error || 'bilinmiyor');

  
      const ordered = [...data.rows].sort((a, b) => Number(b.score) - Number(a.score));
      setRows(ordered);
    } catch (e: any) {
      setErr(e?.message ?? 'bilinmiyor');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="lb">
      <div className="lb__head">
        ...
        <div className="lb__actions">
          <Link href="/" className="btn">Main Menü</Link>
        </div>
      </div>
      ...
      <section className="panel">
        ...
        <div className="meta">
          ...
          {rows.length > 0 && (
            <>
              <span className="sep">•</span>
              <span className="count">{rows.length} oyuncu</span>
            </>
          )}
        </div>
          ...
          <table className="table">
            ...
            <tbody>
              ...
              {rows.map((r, i) => {
                ...
                const name =
                  (typeof r.username === 'string' && r.username) || shorten(addr);
                return (
      ...
      <style jsx>{`
        ...
        /* .btn sınıfı globals.css’ten geliyor */
      `}</style>
    </div>
  );
}
